...
public class SymbolTable {
    ...
    public void exitScope(AnalyzerPrinter printer) {
        for (int i=0; i < currentScope; i++) {
            for (int j=0; j<4; j++) {
                System.out.print(" ");
            }
    ...
    public void printTable(AnalyzerPrinter printer) {
        if (currentScope == -1) {
            return;
        }
        for (int i=0; i < currentScope; i++) {
        ...
        if (table.isEmpty()) {
            ...
            for (Map.Entry<String, SymbolEntry> entry : table.entrySet()) {
				...
				printer.printLevel(entry.getKey() + " -> " + entry.getValue(), currentScope);
            }
        }
    }
